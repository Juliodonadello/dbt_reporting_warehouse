WITH LEASES AS (
  ...
  FROM {{ var('leases_table') }} as LEASES_
	...
	LEFT OUTER JOIN {{ var('lease_deposits_table') }} LEASES_D
		...
		AND (LEASES_D."deleted_at" >= @AsOfDate OR LEASES_D."deleted_at" IS  NULL)
	LEFT OUTER JOIN {{ var('tenants_table') }} TENANTS_
		ON LEASES_."primaryTenantId" = TENANTS_."id" 
  
	WHERE 
		(	(LEASES_."start" <= @AsOfDate AND LEASES_."end" > @AsOfDate)
  			OR (LEASES_."start" <= @AsOfDate AND LEASES_."end" IS NULL)
		)
		--AND LEASES_."status" = 'current'
		AND (LEASES_."deleted_at" >= @AsOfDate OR LEASES_."deleted_at" IS NULL)
		AND CASE WHEN CAST(LEASES_."month_to_month" AS TEXT) ='true' THEN 'True' ELSE 'False' END IN (@month_to_month) 
  	
  GROUP BY 1,2,3,4,5,
		CASE WHEN LEASES_."status" = 'current' THEN 'OCCUPIED' ELSE 'VACANT' END ,
